...
1. **`DynamicsProcessing`** (API 28+) is the primary engine. It's set up as an output-gain-only stage — no compression, no EQ curve, no limiting — so the effect is clean, flat-spectrum attenuation applied uniformly across the signal, rather than anything that reshapes the sound.
