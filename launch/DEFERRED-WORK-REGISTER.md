...
| A2c | 🔴 MANDATORY, SAME BUILD AS A2b — Add Legal links | `MainActivity.kt` (no About screen exists) | Two text links, "Terms of Use" and "Privacy Policy", opening the external browser. **⚠️ COLD-VERIFIED 2026-07-10: there is no About/Settings screen anywhere in the app.** `MainActivity.kt` is a single-screen, flash-and-finish activity (grant permissions → start service → finish()). Placement is an OPEN DESIGN QUESTION, asked to the user 2026-07-10 and explicitly deferred, not answered — resolve it at execution time, not before: either (a) a small footer row of text links added directly to the existing `activity_main.xml` (minimal, no new screen), or (b) a genuinely new About screen (bigger, but gives Restore-purchase — see runbook step 19, same unresolved gap — a proper home too). Decide this together with step 19's placement, since both need somewhere to live and it is wasteful to solve them separately. |
...
|---|---|---|---|
| O0 | Entity separation, personal liability shield | User + accountant | **User's own explicit decision, 2026-07-10: he will handle this himself, later, once there are steady revenue and a real mass of users — not before.** A third-party legal review the same day correctly flagged that operating as a private individual exposes personal assets to a judgment; a registered company (ח.פ / LLC) caps exposure at company assets. Correctly identified as the single most consequential unresolved item, but it is the user's own business/tax decision to make and time, not a task for me to schedule against a specific milestone. Do not raise this again unless the user brings it up, or unless a Pro-launch trigger conversation naturally surfaces it. Revisit the "no legal-entity number to cite in the privacy policy" note in [[granular-volume-pro-plan]] if it ever happens. |
