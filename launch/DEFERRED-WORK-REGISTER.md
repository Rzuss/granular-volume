...
Status: application submitted 2026-07-09 at 2:34 PM. Google says usually 7 days or less.
Watch for the email to the account owner, and the Production card on the Play Console dashboard.

| # | Item | Source of truth | Notes |
|---|---|---|---|
| A1 | Run the launch day review blitz | `launch/LAUNCH-PLAN.md` Phase 1 | Tester "it's live" email first, then replies on the warm Reddit threads that already have engagement, then the cold channel list. Order matters, it is fastest reviews per minute. |
| A2 | Publish the Terms of Use | `store-assets/terms-of-use-PRO-READY.html` (publish everything except sections 5-6) | Drafted in full 2026-07-10 after third-party legal review: Disclaimer of Warranty, Limitation of Liability (both ALL CAPS, UCC conspicuousness), Governing law, plus Indemnification, Severability, Entire Agreement, no-support-obligation, and a medical/injury disclaimer with an assumption-of-risk clause (section 2). Publish to `Rzuss/granular-volume-privacy` as a second document. Add a second footer link next to Privacy Policy on the landing page. |
| A2b | Build the clickwrap consent gate | `MainActivity.kt` existing onboarding/permission screen | **Retimed 2026-07-10, moved up from Pro launch.** The medical/injury disclaimer protects the free tier too, so consent must be captured before production ships, not at Pro. Add a short consent line + "Agree & Continue" above the existing permission card; it must be tapped before "GRANT OVERLAY PERMISSION" becomes active. Persist `KEY_TERMS_ACCEPTED_VERSION` (a version number, not a boolean, so a future material change can re-prompt). Does not touch the floating overlay UI at all. |
| A2c | Add a Legal section to the About screen | `MainActivity.kt` About screen | Two text links, "Terms of Use" and "Privacy Policy", opening the external browser. No need to embed the full text in the APK. |
| A3 | Reply to Omegv10 on Reddit | Drafted, not sent | He reported he could not leave a review. Root cause was the production gate, which this trigger resolves. Draft exists; user posts it. |
| A4 | Name consistency sweep | Several | The Play listing already says `Volume Control: Quiet Dial` (renamed 2026-07-09). Still saying "Granular Volume": `README.md`, `docs/index.html`, `fastlane/metadata/.../title.txt`, launch posts. Deliberately deferred, low urgency, do not bundle with anything risky. |
...
|---|---|---|---|
| O0 | Entity separation, personal liability shield | User + accountant | A third-party legal review 2026-07-10 correctly flagged that operating as a private individual exposes personal assets to a judgment; a registered company (ח.פ / LLC) caps exposure at company assets. This is the single most consequential item raised, but it is a business/tax decision with real ongoing cost, not a text change. Tie it to trigger C (real revenue), not before. Revisit the "no legal-entity number to cite in the privacy policy" note in [[granular-volume-pro-plan]] if this happens. |
| O0b | Tech E&O / cyber insurance | User | Same review, correctly framed as a growth-stage item once paying users reach critical mass, not needed at launch. |
